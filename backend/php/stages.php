...
function ensure_lead_stages_runtime_schema(PDO $db): void {
    try { $db->exec("CREATE TABLE IF NOT EXISTS crminternet_lead_stages (id VARCHAR(40) PRIMARY KEY, name VARCHAR(80) NOT NULL UNIQUE, color VARCHAR(20) NOT NULL DEFAULT 'muted', position INT NOT NULL DEFAULT 0) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"); } catch (Throwable $e) {}
    foreach ([
...
function row_to_lstage(array $r): array {
    return [
        'id'         => $r['id'],
        'name'       => $r['name'],
        'color'      => $r['color'],
...
if ($method === 'GET') {
    $rows = $db->query('SELECT * FROM crminternet_lead_stages ORDER BY position, id')->fetchAll();
    ok(['stages' => array_map('row_to_lstage', $rows)]);
}
