              ...
              <div className="divide-y divide-border">
                {myLeads.map((p) => (
                  <div key={p.id} className="p-4 flex flex-wrap items-center gap-3 hover:bg-muted/20 transition-base">
                    ...
                    {canEditOutcome && <WonDialog currency={currency} onConfirm={(premium, partner) => { markWon(p.id, premium, partner); toast.success("Contrat enregistré", { description: `${p.firstName} ${p.lastName} • ${formatAmount(premium, currency)}` }); }} />}
                    {canEditOutcome && <SharedLostDialog onConfirm={(payload) => { markLost(p.id, payload.formatted); toast("Lead marqué perdu", { description: payload.formatted }); }} />}
